...
export default function AnalyticsPage() {
  ...
  const fetchAnalyticsData = async () => {
    try {
      ...
      const [monthlyRes, dailyRes, categoryRes, paymentRes] = await Promise.all([
        DashboardService.getMonthlyRevenue({ year: selectedYear }), // Ensure this matches API signature
        DashboardService.getDailyRevenue({ month: selectedMonth, year: selectedYear }),
        DashboardService.getSalesByCategory(),
        DashboardService.getPaymentMethodStats(),
      ]);
      ...
      // Safe Data Setting
      setMonthlyRevenue(Array.isArray(monthlyRes.data.data) ? monthlyRes.data.data : []);
      setDailyRevenue(Array.isArray(dailyRes.data.data) ? dailyRes.data.data : []);
      setCategorySales(Array.isArray(categoryRes.data.data) ? categoryRes.data.data : []);
      setPaymentMethods(Array.isArray(paymentRes.data.data) ? paymentRes.data.data : []);
  ...
  return (
    <div className="space-y-6 pb-10">
      ...
      >
        <div>
          <h1 className="text-3xl font-bold text-white">Analytics</h1>
          <p className="mt-1 text-gray-400">Detailed insights and performance metrics</p>
        </div>
        ...
        <div className="flex items-center gap-3 bg-white/5 p-1 rounded-xl border border-white/10">
          ...
          <select
            value={selectedYear}
            onChange={(e) => setSelectedYear(parseInt(e.target.value))}
            className="bg-transparent text-sm text-white px-2 py-1.5 focus:outline-none cursor-pointer"
          >
            {[2024, 2023, 2022].map(year => (
      ...
      <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-4">
        {[
          ...
          { label: 'Total Orders', value: totalOrders.toString(), icon: ShoppingCart, color: 'text-blue-400', bg: 'bg-blue-500/10' },
          { label: 'Avg Order Value', value: formatCurrency(avgOrderValue), icon: TrendingUp, color: 'text-purple-400', bg: 'bg-purple-500/10' },
          { label: 'Active Categories', value: categorySales.length.toString(), icon: Package, color: 'text-cyan-400', bg: 'bg-cyan-500/10' },
        ].map((kpi, index) => (
          <motion.div
            key={kpi.label}
            initial={{ opacity: 0, y: 20 }}
      ...
      <div className="grid gap-6 lg:grid-cols-3">
          ...
          <div className="h-[350px] w-full">
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={monthlyRevenue}>
                ...
                <XAxis dataKey="month" stroke="#9ca3af" style={{ fontSize: '12px' }} />
                <YAxis stroke="#9ca3af" style={{ fontSize: '12px' }} tickFormatter={(val) => `₹${val/1000}k`} />
                <Tooltip
        ...
        >
          <h3 className="mb-6 text-lg font-semibold text-white">Payment Methods</h3>
          <div className="h-[350px] w-full flex items-center justify-center">
            <ResponsiveContainer width="100%" height="100%">
              <PieChart>
                ...
                >
                  {paymentMethods.map((entry, index) => (
                    <Cell key={`cell-${index}`} fill={entry.method === 'Razorpay' ? '#10b981' : '#f59e0b'} stroke="rgba(0,0,0,0)" />
                  ))}
                </Pie>
                <Tooltip 
      ...
      {/* Charts Row 2: Category Performance */}
      <motion.div
        initial={{ opacity: 0, y: 20 }}
        animate={{ opacity: 1, y: 0 }}
        transition={{ delay: 0.4 }}
        className="rounded-2xl border border-white/10 bg-white/[0.02] p-6"
      >
        <h3 className="mb-6 text-lg font-semibold text-white">Sales by Category</h3>
        <div className="h-[350px] w-full">
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={categorySales} margin={{ top: 20, right: 30, left: 20, bottom: 5 }}>
              <CartesianGrid strokeDasharray="3 3" stroke="rgba(255,255,255,0.1)" vertical={false} />
      ...
      >
        <div className="p-6 border-b border-white/10">
          <h3 className="text-lg font-semibold text-white">Detailed Category Report</h3>
        </div>
        <div className="overflow-x-auto">
          <table className="w-full">
            <thead className="bg-white/5">
              <tr>
                ...
                <th className="px-6 py-4 text-left text-xs font-semibold text-gray-400 uppercase">Total Sales</th>
                ...
                <th className="px-6 py-4 text-left text-xs font-semibold text-gray-400 uppercase">Avg Order</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-white/10">
              {categorySales.map((cat, idx) => (
                <tr key={idx} className="hover:bg-white/[0.02] transition-colors">
                  ...
                  <td className="px-6 py-4 text-sm text-gray-300">{cat.totalSales}</td>
                  <td className="px-6 py-4 text-sm font-semibold text-green-400">{formatCurrency(cat.revenue)}</td>
                  <td className="px-6 py-4 text-sm text-gray-300">{cat.productCount}</td>
                  <td className="px-6 py-4 text-sm text-gray-300">{formatCurrency(cat.totalSales > 0 ? cat.revenue / cat.totalSales : 0)}</td>
                </tr>
              ))}
            </tbody>
          </table>
